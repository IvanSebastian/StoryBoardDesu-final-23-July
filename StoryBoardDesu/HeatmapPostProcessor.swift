...
class HeatmapPostProcessor {
    func convertToPredictedPoints(from heatmaps: MLMultiArray,isInverse: Bool = false) -> [PredictedPoint?] {
        ...
        n_kpoints = n_kpoints.map { kpoint -> PredictedPoint? in
            if let kp = kpoint {
                ...
                return PredictedPoint(maxPoint: CGPoint(x: isInverse ? 1 - (kp.maxPoint.x+0.5)/CGFloat(heatmap_w) : (kp.maxPoint.x+0.5)/CGFloat(heatmap_w),
